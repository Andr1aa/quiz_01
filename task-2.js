...
const filterExpensiveProducts=(products, priceThreshold) =>
...
const applyDiscount = (products, discountPercentage) =>
  products.map(({ name, price, stock }) => 
    ({name,price: price * (1 - discountPercentage/100),stock}));


const getAvailableProducts =(products) => products.filter(({stock}) => stock > 0).map(({name,price}) => ({name, price}));
